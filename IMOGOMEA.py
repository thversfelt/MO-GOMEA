...
class IMOGOMEA:
    ...
    def evolve(self):
        ...
        while self.evaluations < self.maxEvaluations:
            ...
            for solution in self.population:
                ...
                if cluster in self.extremeClusters and len(self.clusters) > 1:
    ...
    def createRandomSolution(self):
        ...
        while not self.problem.isFeasibleSolution(randomSolution):
            randomSolution = Solution(util.randomGenotype(self.problem.problemSize))
    ...
    def evaluateFitness(self, solution):
        ...
        if self.evaluations % 1000 == 0 and self.evaluations <= self.maxEvaluations:
            self.progression[self.evaluations] = self.elitistArchive[self.currentGeneration]

        if not self.problem.isFeasibleSolution(solution):
